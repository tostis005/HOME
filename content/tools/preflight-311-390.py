...
def load(n):
 p=next(T.glob(f'batch-*{n//5*5 if False else ""}*.json'),None)
 for q in T.glob('batch-*.json'):
  data=json.loads(q.read_text(encoding='utf-8'))
  for item in data:
   if item['n']==n: return q,data,item
 raise SystemExit(f'missing item {n}')

def save(q,data): q.write_text(json.dumps(data,ensure_ascii=False,separators=(',',':'))+'\n',encoding='utf-8')

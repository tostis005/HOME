...
# Required acronym/context checks in Spanish final prose.
...
# Exact FAQ duplicates are a visible-template failure.
for lang in ('es','en'):
 dup=[q for q,c in Counter(qs[lang]).items() if c>1]
 if dup: errs.append(f'{lang}: duplicate FAQ questions: {dup[:5]}')
# Safety stop concepts that must survive final generation.
